...
//					Con_Printf("fingerprint = \"%08x\"\n", BigLong(*(int*)(attr+1)));
			...
			else
			{
				if (integritypos)
				{
					...
					HMAC_fte(SHA1_m, key, sizeof(key), (qbyte*)stun, integritypos-4 - (char*)stun, con->lpwd, strlen(con->lpwd));
				...
				if (con->state != ICE_INACTIVE)
				{
					...
					//but the hash is to the start of the attribute's header
					HMAC_fte(SHA1_m, integrity, sizeof(integrity), data, buf.cursize, con->lpwd, strlen(con->lpwd));
					MSG_WriteShort(&buf, BigShort(0x8));	//MESSAGE-INTEGRITY
